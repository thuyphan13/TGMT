...
class YoloDetect():
    ...
    def draw_prediction(self, img, class_id, x, y, x_plus_w, y_plus_h, points):
        ...
        isInside = False
        for point in points:
            if (point[0] >= x) and (point[0] <= x_plus_w) and (point[1] >= y) and (point[1] <= y_plus_h):
                isInside = True
                break
    ...
    def alert(self, img):
        ...
        # Kiểm tra thời gian giữa hai lần cảnh báo
        if (self.last_alert is None) or (
                (datetime.datetime.utcnow() - self.last_alert).total_seconds() > 5):
            self.last_alert = datetime.datetime.utcnow()
            
            # Lưu lại khung hình
            filename = "alert/alert_{}.png".format(datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
            cv2.imwrite(filename, cv2.resize(img, dsize=None, fx=3, fy=3))
